...
def train_word2vec(filenames):
    ...
    '''
    t1 = time.time()
    # sentences = Get_Sentences(filenames)###原来的
    # print(sentences)

    news = open('F:\\text-cnn-ENG_ASK\\data\\RESULT_raw.txt', 'r', encoding='utf-8')

    logging.basicConfig(format='%(asctime)s : %(levelname)s : %(message)s', level=logging.INFO)
    model = word2vec.Word2Vec(LineSentence(news), size=100, window=5, min_count=2, workers=6)
